...
export class RhiUiSelectableGrid extends HTMLElement {
    [key: string]: any;
    public static get is(): string { return 'rhi-ui-selectable-grid'; }

    private static rectanglesIntersect(
        r1Top: number, r1Right: number, r1Bottom: number,
        r1Left: number, r2Top: number, r2Right: number, r2Bottom: number, r2Left: number): boolean {
    ...
    public static get observedAttributes(): string[] {
        ...
        for (const key in RhiUiSelectableGrid.properties) {
            if (RhiUiSelectableGrid.properties.hasOwnProperty(key)) {
    ...
    public connectedCallback(): void {
        ...
        // Other wise we asume the object was extended and the rows were added directly
        // to the element's template instead of <slot>.
    ...
    public getCellValue(name: string): string {
        const cell: HTMLElement = this.cells.find((c: HTMLElement) => c.getAttribute('name') === name) as HTMLElement;
        const value = cell ? cell.getAttribute('value') : '';
        return value ? value : '';
    }
    ...
    private initializeCells(rows: HTMLCollection): void {
        ...
        for (let r: number = 0; r < rows.length; r++) {
            const row: HTMLElement = rows.item(r) as HTMLElement;
            ...
            for (let c: number = 0; c < row.children.length; c++) {
                if (row.children[c].tagName.toUpperCase() === 'RHI-UI-SELECTABLE-GRID-CELL') {
                    this.cells.push(row.children.item(c) as RhiUiSelectableGridCell);
                }
            }
        }
    }
    ...
    private updateRange(top: number, right: number, bottom: number, left: number): void {
        ...
        const currentValue: string | null = this.selectedCell ? this.selectedCell.getAttribute('value') : null;

        this.cells.forEach((c: HTMLElement) => {
            const bounds = c.getBoundingClientRect();
            if (
                RhiUiSelectableGrid.rectanglesIntersect(bounds.top, bounds.left + bounds.width,
                    bounds.top + bounds.height, bounds.left, top, right, bottom, left)
    ...
    private trackHandler(e: any): void {
    ...
    private getCellFromElementPath(element: any): HTMLElement | null {
